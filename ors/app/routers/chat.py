...
async def chat(request: Request, body: ChatRequest, 
    ...
    try:
        ...
        response_message = f"The free model {orouter_llm.model_name} is temporarily rate-limited. Please try again in a moment."
        try:
...
async def summarize(
    ...
    factory = request.app.state.llm_factory
    llm = factory.get_llm(body.model_id, body.free_models)
    prompt_kwargs = {"text": body.text}
    summary = invoke_llm(llm, prompt_type="summarization", prompt_kwargs=prompt_kwargs)
